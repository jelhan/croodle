...
class DataHandler
{
    ...
    public function set()
    {
        ...
        // write data
        if (!$this->_writeData($this->request->data)) {
            return $this->result;
        }
     ...
     */
    protected function _traficLimiterCanPass()
    {
        if (self::TRAFIC_LIMITER === 0) {
            return true;
        }
        ...
        if (is_writeable($filename) === false) {
            throw new Extension ("File for IP list storage is not writeable. Check permissions.");
        }
        ...
        if (isset($ip_list[$ip]) && (($ip_list[$ip] + self::TRAFIC_LIMITER) > time())) {
            return false;
        }
        
        // delete expired IPs
        foreach ($ip_list as $ip => $time) {
            if (($time + self::TRAFIC_LIMITER) < time()) {
                unset($ip_list[$ip]);
            }
        }
    ...
    protected function _writeData($data)
    {
        ...
        if (!$this->_writeDatum('head', json_encode($data->head))) {
            return false;
        }
        if (!$this->_writeDatum('data', $data->data)) {
            return false;
        }
        
        $i = 0;
        foreach ($data->user as $user) {
            if(!$this->_writeDatum('user_'.$i, $user)) return false;
            $i++;
        }
